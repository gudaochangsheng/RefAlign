...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, timestep):
        cond_size = self.cond_size
        gate = 1.0
        if cond_size is not None and self.decoubed_subject and cond_size != 1560:
            L = x.shape[1]
            L_main = L - cond_size
            T = 1560
            assert cond_size % T == 0, f"cond_size={cond_size} must be multiple of {T}"
            S = cond_size // T
            ip_start = L_main
            ...
            # RoPE on full sequence (freqs aligned)
            q = rope_apply(q, freqs, self.num_heads)
            k = rope_apply(k, freqs, self.num_heads)
            ...
            # 1) main attends to (main + all ip)
            out[:, :L_main] = self.attn(q_main, k, v)  # Q=main, KV=full

            # 2) each ip attends only to itself (no main, no other ip)
            for s in range(S):
                s0 = ip_start + s * T
                s1 = s0 + T
                q_s = q[:, s0:s1]
                k_s = k[:, s0:s1]
                v_s = v[:, s0:s1]
                out[:, s0:s1] = self.attn(q_s, k_s, v_s)  # Q=ip_s, KV=ip_s
...
class DiTBlock(nn.Module):
    ...
    def init_inputmlp(self, train=False):
        ...
        self.input_mlp = MLP3(dim, dim // 2,1024, has_pos_emb=False)
        # self.input_mlp = MLP3(dim, dim // 2,1152, has_pos_emb=False)
        # self.input_mlp = MLP3(dim, dim // 2,1280, has_pos_emb=False)
        # self.input_mlp = MLP3(dim, dim // 2,3584, has_pos_emb=False)
        for p in self.input_mlp.parameters():
            p.requires_grad = bool(train)

    def forward(self, x, context, t_mod, freqs, x_ip, cond_size, image_num,t_mod_ip, timestep, decoubed_subject):
        if cond_size is None:
            ...
            self.self_attn.cond_size = cond_size
            self.self_attn.decoubed_subject = decoubed_subject
            # self.self_attn.image_num = image_num
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                self.modulation.to(dtype=t_mod.dtype, device=t_mod.device) + t_mod).chunk(6, dim=1)
            input_x = modulate(self.norm1(x), shift_msa, scale_msa)
            ...
            x = torch.concat([x_video, x_ip], dim=1)
            input_x = modulate(self.norm2(x), shift_mlp, scale_mlp)
            x = self.gate(x, gate_mlp, self.ffn(input_x))
            x_ip = x[:,-cond_size:]
            return x,x_ip
...
class WanModel(torch.nn.Module):
    ...
    def init_inputmlp(self, train=False):
        dim = self.dim
        self.input_mlp = MLP3(dim, dim // 2,768, has_pos_emb=False)  #DINOv3-L 1024 Qwen2.5VL 3584 SigLIP 1280
